...
def plot_scGPT_umap(file1,file2 , output_pdf=None, skip_preprocessing=False):
    ...
    adata.obsm["umap"] = umap_model.fit_transform(adata.obsm["X_scGPT"])

    # For new data, use the same UMAP model to ensure consistent embedding
    new.obsm["umap"] = umap_model.transform(new.obsm["X_scGPT"])

    # Step 3: Run KNN classification using scGPT embeddings
    # Using scGPT latent representations
    knn_scGPT = KNeighborsClassifier()
    knn_scGPT.fit(adata.obsm["X_scGPT"], adata.obs["labels"].values.astype(str))
    scGPT_accuracy = accuracy_score(knn_scGPT.predict(new.obsm["X_scGPT"]), new.obs["labels"].values.astype(str))
    print(f"KNN accuracy using scGPT embeddings: {scGPT_accuracy:.4f}")

    # Using UMAP embeddings derived from scGPT
    ...
    # Get the obsm keys (embeddings like 'X_scGPT', 'umap', etc.)
    ...
    utils.plot(adata.obsm["umap"], adata.obs["labels"], ax=ax[0], title="Reference embedding (UMAP from scGPT)", 
            ...
            legend_kwargs=dict(loc="upper center", bbox_to_anchor=(0.5, 0.05), 
                                ...
                                knn_scGPT_accuracy=scGPT_accuracy,  # Pass the accuracies
    ...
    fig.text(0.5, 0.10, f"KNN(scGPT): {scGPT_accuracy:.4f}    |    KNN(UMAP): {umap_accuracy:.4f}", 
    ...
    if output_pdf is None:
        ...
        output_pdf = f"umap_plot_scGpt_Knn_concatenated{file1_name}_{file2_name}.pdf"
    
    plt.savefig(output_pdf, dpi=600, bbox_inches="tight", transparent=True)

if __name__ == "__main__":
    
    # plot_scGPT_umap("Datasets/baron_2016h.h5ad", "Datasets/xin_2016.h5ad")
    
    # plot_scGPT_umap("Datasets/baron_2016h_scGPT.h5ad", "Datasets/xin_2016_scGPT.h5ad", skip_preprocessing=True)
