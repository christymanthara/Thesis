...
def plot_scanvi_umap(file1,file2 , output_pdf=None, skip_preprocessing=False):
    ...
    adata.obsm["umap"] = umap_model.fit_transform(adata.obsm["X_scANVI"])

    # For new data, use the same UMAP model to ensure consistent embedding
    new.obsm["umap"] = umap_model.transform(new.obsm["X_scANVI"])
    ...
    knn_scanvi.fit(adata.obsm["X_scANVI"], adata.obs["labels"].values.astype(str))
    scanvi_accuracy = accuracy_score(knn_scanvi.predict(new.obsm["X_scANVI"]), new.obs["labels"].values.astype(str))
    print(f"KNN accuracy using scanvi embeddings: {scanvi_accuracy:.4f}")

    # Using UMAP embeddings derived from scanvi
    ...
    # Get the obsm keys (embeddings like 'X_scANVI', 'umap', etc.)
    ...
    utils.plot(adata.obsm["umap"], adata.obs["labels"], ax=ax[0], title="Reference embedding (UMAP from scanvi)", 
    ...
    fig.text(0.5, 0.10, f"KNN(scanvi): {scanvi_accuracy:.4f}    |    KNN(UMAP): {umap_accuracy:.4f}", 
    ...
    if output_pdf is None:
        ...
        output_pdf = f"umap_plot_scanVI_Knn_concatenated{file1_name}_{file2_name}.pdf"
    
    plt.savefig(output_pdf, dpi=600, bbox_inches="tight", transparent=True)

if __name__ == "__main__":
    
    plot_scanvi_umap("Datasets/baron_2016h.h5ad", "Datasets/xin_2016.h5ad")
    
    # plot_scanvi_umap("Datasets/baron_2016h_scanvi.h5ad", "Datasets/xin_2016_scanvi.h5ad", skip_preprocessing=True)
    # plot_scanvi_umap("Datasets/hrvatin_2018_scanvi.h5ad", "Datasets/chen_2017_scanvi.h5ad", skip_preprocessing=True)
