...
import pickle
import json
from .knn_add_excel_table import integrate_excel_export

def create_results_table(results_table, ref_source: str, query_source: str, base_filename, metadata=None):
    """
    ...
    query_source : str
        Name of the query source dataset
    base_filename : str
        Base filename for output
    metadata : dict, optional
        Dictionary containing tissue and organism information
        Expected keys: ref_tissue, query_tissue, ref_organism, query_organism, 
                      ref_cell_count, query_cell_count
    """
    ...
    if metadata:
        ...
        query_tissue = metadata.get('query_tissue', 'unknown')
        ref_organism = metadata.get('ref_organism', 'unknown')
        query_organism = metadata.get('query_organism', 'unknown')
        ref_cell_count = metadata.get('ref_cell_count', 'unknown')
        query_cell_count = metadata.get('query_cell_count', 'unknown')
        
        # Create enhanced comparison key
        source_comparison_key = f"{ref_source} vs {query_source}"
        
        # Create metadata string for display
        metadata_str = f"Ref: {ref_organism}_{ref_tissue}({ref_cell_count}) | Query: {query_organism}_{query_tissue}({query_cell_count})"
    ...
    create_enhanced_table_visualization(df_query, df_reference, accumulated_results, 
                                      source_comparison_key, base_filename)
...
def create_enhanced_table_visualization(df_query, df_reference, accumulated_results, 
                                      current_comparison, base_filename):
...
def create_metadata_dataframe(accumulated_results):
    ...
    for comparison, data in accumulated_results.items():
        ...
        metadata_data['Query Source'].append(metadata.get('query_source', 'unknown'))
        metadata_data['Reference Tissue'].append(metadata.get('ref_tissue', 'unknown'))
        metadata_data['Query Tissue'].append(metadata.get('query_tissue', 'unknown'))
        metadata_data['Reference Organism'].append(metadata.get('ref_organism', 'unknown'))
        metadata_data['Query Organism'].append(metadata.get('query_organism', 'unknown'))
        metadata_data['Reference Cell Count'].append(metadata.get('ref_cell_count', 'unknown'))
        metadata_data['Query Cell Count'].append(metadata.get('query_cell_count', 'unknown'))
...
if __name__ == "__main__":
    # Test the enhanced functionality with new metadata structure
    
    # Example metadata for second set
    metadata_1 = {
        'ref_source': 'dataset1',
        'query_source': 'dataset2',
        'ref_tissue': 'pancreas',
        'query_tissue': 'pancreas',
        'ref_organism': 'human',
        'query_organism': 'human',
        'ref_cell_count': 5000,
        'query_cell_count': 3000
    }
    
    metadata_2 = {
        'ref_source': 'dataset3',
        'query_source': 'dataset4',
        'ref_tissue': 'pancreas',
        'query_tissue': 'pancreas',
        'ref_organism': 'mouse',
        'query_organism': 'human',
        'ref_cell_count': 4500,
        'query_cell_count': 3500
    }
    
    # Example 1: First run with new metadata structure
    print("=== Example 1: First run with new metadata structure ===")
    results_table_1 = {
        'scVI': {'Reference CV': '0.850', 'Query Transfer': '0.834'},
        'scANVI': {'Reference CV': '0.901', 'Query Transfer': '0.867'},
        'RAW data': {'Reference CV': '0.612', 'Query Transfer': '0.598'}
    }
    create_results_table(results_table_1, "dataset1", "dataset2", "test_file_1", metadata=metadata_1)
    
    # Example 2: Second run with different metadata
    print("\n=== Example 2: Second run with different metadata ===")
    results_table_2 = {
        'scVI': {'Reference CV': '0.782', 'Query Transfer': '0.756'},
        'scANVI': {'Reference CV': '0.823', 'Query Transfer': '0.801'},
        'UMAP': {'Reference CV': '0.689', 'Query Transfer': '0.654'}
    }
    create_results_table(results_table_2, "dataset3", "dataset4", "test_file_2", metadata=metadata_2)
    
    # Show all accumulated results
    print("\n=== Final accumulated results with new metadata structure ===")
