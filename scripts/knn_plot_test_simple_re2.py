...
def make_plot(adata_with_tsne, tsne_key, embedding_name, 
              draw_centers=False, draw_cluster_labels=False, 
              center_size=48, fontsize=8):
    """
    Enhanced plotting function - splits t-SNE embeddings by source for visualization
    with optional cluster centers and labels
    
    Parameters:
    -----------
    adata_with_tsne : AnnData
        AnnData object containing t-SNE embeddings
    tsne_key : str
        Key for t-SNE coordinates in adata.obsm
    embedding_name : str
        Name of the embedding for plot titles
    draw_centers : bool, default False
        Whether to draw cluster centers
    draw_cluster_labels : bool, default False
        Whether to draw cluster labels (requires draw_centers=True)
    center_size : int, default 48
    ...
    # Add cluster centers and labels for source 1
    if draw_centers:
        source1_centers = []
        source1_center_labels = []
        for label in unique_labels:
            label_mask = source1_labels == label
            if label_mask.any():
                center = np.median(source1_coords[label_mask], axis=0)
                source1_centers.append(center)
                source1_center_labels.append(label)
                ax1.scatter(center[0], center[1], c=[color_map[label]], 
                           s=center_size, alpha=1, edgecolor="k", linewidth=2)
                
                if draw_cluster_labels:
                    ax1.text(center[0], center[1] + 2.2, str(label), 
    ...
    if len(unique_sources) > 1:
        ...
        # Plot reference data as background (light grey)
        for i, label in enumerate(unique_labels):
            label_mask = source1_labels == label
            if label_mask.any():
        ...
        # Plot query data with full colors
        for i, label in enumerate(unique_labels):
        ...
        # Add cluster centers and labels for source 2
        if draw_centers:
            # First add reference centers in grey
            if 'source1_centers' in locals():
                for center, label in zip(source1_centers, source1_center_labels):
                    ax2.scatter(center[0], center[1], c='lightgrey', 
                               s=center_size, alpha=0.5, edgecolor="darkgrey", linewidth=1)
            
            # Then add query centers with full colors
            source2_centers = []
            source2_center_labels = []
            for label in unique_labels:
                label_mask = source2_labels == label
                if label_mask.any():
                    center = np.median(source2_coords[label_mask], axis=0)
                    source2_centers.append(center)
                    source2_center_labels.append(label)
                    ax2.scatter(center[0], center[1], c=[color_map[label]], 
                               s=center_size, alpha=1, edgecolor="k", linewidth=2)
                    
                    if draw_cluster_labels:
                        ax2.text(center[0], center[1] + 2.2, str(label), 
                                fontsize=fontsize, horizontalalignment="center",
                                weight='bold', color='black')
        ...
        # Add cluster centers for single source case
        if draw_centers and 'source1_centers' in locals():
            for center, label in zip(source1_centers, source1_center_labels):
                ax2.scatter(center[0], center[1], c=[color_map[label]], 
                           s=center_size, alpha=1, edgecolor="k", linewidth=2)
    ...
    # Create filename with source names
    source1_name = unique_sources[0] if len(unique_sources) > 0 else "source1"
    source2_name = unique_sources[1] if len(unique_sources) > 1 else "source2"
    
    # Clean the names to be filesystem-safe
    source1_clean = str(source1_name).replace('/', '_').replace('\\', '_')
    source2_clean = str(source2_name).replace('/', '_').replace('\\', '_')
    embedding_clean = str(embedding_name).replace('/', '_').replace('\\', '_')
    
    # Add centers info to filename if enabled
    centers_suffix = "_with_centers" if draw_centers else ""
    labels_suffix = "_with_labels" if draw_cluster_labels else ""
    
    filename = f"plot_{source1_clean}_{source2_clean}_{embedding_clean}{centers_suffix}{labels_suffix}.pdf"
