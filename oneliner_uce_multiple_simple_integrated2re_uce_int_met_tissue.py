...
from harmony.test_harmony_cells import run_harmony_correction_simple, run_harmony_correction_with_original_x
from simple_code import knn_plot_test_simple
...
def process_tissue_group_int(tissue, file_info_list, enable_uce=True, 
                        uce_config=None, batch_key='batch',
                        enable_pavlin=True,  # NEW
                        enable_integrated=True,  # NEW
                        analysis_mode='standard'):  # NEW
    ...
    for i in range(len(file_info_list) - 1):
        ...
        # Initialize return variables for this iteration
        integrated_results = {}
        df_query = None
        df_reference = None
        metadata = {}
        ...
        try:
            ...
            if not os.path.exists(file1_path):
                print(f"❌ Reference file not found: {file1_path}")
                continue
            
            if not os.path.exists(file2_path):
                print(f"❌ Query file not found: {file2_path}")
                continue
            ...
            uce_file1 = anndata.read_h5ad(processed_file1)
            uce_file2 = anndata.read_h5ad(processed_file2)

            # Add source labels
            uce_file1.obs["source"] = pd.Categorical([source1_name] * uce_file1.n_obs)
            uce_file2.obs["source"] = pd.Categorical([source2_name] * uce_file2.n_obs)

            # Debug print to check source labels
            print("Unique source labels in adata:", uce_file1.obs["source"].unique())
            print("Unique source labels in new:", uce_file2.obs["source"].unique())
            ...
            try:
                print("🔄 Running KNN and t-SNE analysis on UCE output...")
                results0, metadata0 = compute_knn_tsne_simple_with_transformation_plots(uce_file1, reference_file=uce_file2)
            ...
            # Use original dataset paths instead of UCE filepath
            combined_data = None
            try:
                print("🔄 Loading and preprocessing datasets...")
            ...
            if enable_integrated:
                ...
                integrated_results = results1
                print(f"ℹ️  Using main pipeline results only: {len(integrated_results)} methods")
...
def run_simple_pipeline(dataset1_path, dataset2_path,
                               enable_pavlin=False,
                               ...
                               uce_config=None, 
                               batch_key="source",
                               max_memory_gb=16):  # Add memory limit parameter
    """
    Enhanced pipeline with better memory management and error handling
    """
    
    print("🚀 Starting Enhanced Simple Pipeline (Fixed Version)...")
    ...
    if available_memory_gb < max_memory_gb:
        print(f"⚠️  Warning: Available memory ({available_memory_gb:.1f}GB) < requested ({max_memory_gb}GB)")
    ...
    try:
        # Step 0: Process datasets with UCE (Optional) - with better error handling
        if enable_uce:
            print("\n" + "="*60)
            print("STEP 0: UCE EMBEDDINGS PROCESSING")
            print("="*60)
            ...
            try:
                ...
                if len(uce_processed_files) != 2:
                    print("⚠️  Could not process both datasets with UCE, falling back to original files")
                    file1_path = dataset1_path
                    file2_path = dataset2_path
                else:
                    print(f"✅ UCE processed files:")
                ...
                print(f"❌ Error in UCE processing: {str(e)}")
                print("⚠️  Falling back to original files...")
                file1_path = dataset1_path
                file2_path = dataset2_path
        else:
            print("\nℹ️  Skipping UCE processing, using original files...")
            file1_path = dataset1_path
            file2_path = dataset2_path
        
        # Step 1: Loading and preprocessing with the fixed function
        print("\n" + "="*60)
        print("STEP 1: LOADING AND PREPROCESSING DATASETS (FIXED)")
        print("="*60)
        
        # Use the fixed preprocessing function
        combined_data = preprocessing.load_and_preprocess_multi_embedder_fixed(
            file1=file1_path,
            file2=file2_path,
            save=False,
            split_output=False
        ...
        if combined_data is None:
            raise Exception("Preprocessing failed - combined_data is None")
        
        print(f"📦 Available keys in uns after step 1:")
        print(list(combined_data.uns.keys()) if hasattr(combined_data, 'uns') else "No 'uns' attribute")
        
        # Force garbage collection before next step
        import gc
        gc.collect()
        
        # Step 2: Adding embeddings with better error handling
        print("\n" + "="*60)
        print("STEP 2: ADDING ADDITIONAL EMBEDDINGS (scVI, scANVI, UCE)")
        print("="*60)
        ...
        try:
            ...
            if embedded_adata is None:
                print("⚠️  Embedding step returned None, using original data")
                embedded_adata = combined_data
                
        except Exception as e:
            print(f"❌ Error in embedding step: {str(e)}")
            print("⚠️  Using original data without additional embeddings")
            embedded_adata = combined_data
        ...
        # Step 2.5: Add harmony embedding with error handling
        print("\n" + "="*60)
        print("STEP 2.5: HARMONY CORRECTION")
        print("="*60)
        ...
        try:
            ...
            if adata_harmony is None:
                print("⚠️  Harmony correction returned None, using embedded data")
                adata_harmony = embedded_adata
                
        except Exception as e:
            print(f"❌ Error in harmony correction: {str(e)}")
            print("⚠️  Using embedded data without harmony correction")
            adata_harmony = embedded_adata
        ...
        try:
            results1, metadata1 = compute_knn_tsne_simple_with_transformation_plots(adata_harmony)
            print(f"✅ Analysis completed with {len(results1)} methods")
        except Exception as e:
            print(f"❌ Error in KNN/t-SNE analysis: {str(e)}")
            results1, metadata1 = {}, {}
        
        # Continue with remaining steps...
        return results1, None, None  # Simplified return for now
...
def run_simple_pipeline_safe(dataset1_path, dataset2_path, enable_uce=True, 
                            uce_config=None, batch_key="source"):
    """
    Safe wrapper for run_simple_pipeline with additional error handling
    
    This function ensures that even if the pipeline fails, it returns 
    meaningful information about what went wrong.
    
    Returns:
    --------
    dict: Status dictionary with 'success', 'results', 'error' keys
    """
    ...
    try:
        ...
        results_dict, df_query, df_reference, metadata = results
        ...
        if results_dict is not None:
            ...
            return {
                'success': False,
                'results': None,
                'error': "Pipeline completed but returned no results"
            }
...
def run_simple_pipeline_robust(dataset1_path, dataset2_path,
                               enable_pavlin=False,
                               enable_integrated=False,
                               analysis_mode='standard',
                               enable_uce=True, 
    ...
    try:
        ...
        if results1 and results2:
            ...
            integrated_results = results2
            print(f"ℹ️  Using second pipeline results only: {len(integrated_results)} methods")
        else:
            raise Exception("No results available from either pipeline")
        ...
        traceback.print_exc()
        print("⚠️  Using whatever results are available")
        integrated_results = results1 if results1 else results2
        pipeline_status['results_integration'] = False
...
# Enhanced version with memory monitoring
def run_simple_pipeline_with_monitoring(dataset1_path, dataset2_path, **kwargs):
    """
    ...
    try:
        ...
        results = run_simple_pipeline_robust(dataset1_path, dataset2_path, **kwargs)
...
def main_simple_pipeline():
    """
    Example usage of the enhanced simple pipeline
    """
    ...
    if status['success']:
        ...
        print(f"Methods analyzed: {len(results['results_dict'])}")
        
        if results['df_query'] is not None:
            print(f"Query results available: {results['df_query'].shape}")
        if results['df_reference'] is not None:
            print(f"Reference results available: {results['df_reference'].shape}")
...
def main():
    """
    Main pipeline function with options for both tissue-based and simple processing
    """
    
    # Configuration
    ENABLE_UCE = True  # Set to False to skip UCE processing
    PROCESSING_MODE = "simple"  # Options: "tissue", "simple"
    ...
    if PROCESSING_MODE == "simple":
        ...
        try:
            # Run the enhanced simple pipeline
            # results_dict, df_query, df_reference, metadata = run_simple_pipeline(
            results_dict, df_query, df_reference = run_simple_pipeline(
                dataset1_path=dataset1_path,
                dataset2_path=dataset2_path,
                enable_uce=ENABLE_UCE,
                uce_config=UCE_CONFIG,
            ...
            if results_dict is not None:
                print("\n" + "="*50)
                print("🎉 SIMPLE PIPELINE ANALYSIS COMPLETE!")
                print("="*50)
                ...
                print("\n❌ Simple pipeline completed but returned no results")
    ...
    if PROCESSING_MODE == "simple":
        ...
        try:
            ...
            if ENABLE_MEMORY_MONITORING:
                ...
                results_dict, df_query, df_reference, metadata = run_simple_pipeline_with_monitoring(
                    dataset1_path=dataset1_path,
                    dataset2_path=dataset2_path,
                    enable_uce=ENABLE_UCE,
                    uce_config=UCE_CONFIG,
                ...
                results_dict, df_query, df_reference, metadata = run_simple_pipeline_robust(
                    dataset1_path=dataset1_path,
                    dataset2_path=dataset2_path,
                    enable_uce=ENABLE_UCE,
                    uce_config=UCE_CONFIG,
